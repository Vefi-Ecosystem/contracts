...
// This contracts searches possible shared paths for non-pair tokens in the exchange and prunes them to a 3-way path
contract FabulousExchangeMultiHopsRouter {
  ...
  function getPossiblePath(address tokenA, address tokenB) public view returns (address[] memory) {
  ...
  function swapExactTokensForTokens(
    ...
    address[] memory path = getPossiblePath(tokenA, tokenB);
  ...
  function swapExactETHForTokens(
    ...
    address[] memory path = getPossiblePath(router.WETH(), token);
    ...
    if (path.length == 2) {
      ...
      for (uint256 i = 1; i < path.length; i++) {
        ...
        if (token0 == router.WETH()) {
          router.swapExactETHForTokens{value: msg.value}(AMOUNTOUTMIN, innerPath, recipient, deadline);
        } else {
          router.swapExactTokensForTokens(AMOUNTIN, AMOUNTOUTMIN, innerPath, recipient, deadline);
        }
      }
  ...
  function swapExactETHForTokensSupportingFeeOnTransferTokens(
    ...
    if (path.length == 2) {
      ...
      for (uint256 i = 1; i < path.length; i++) {
        ...
        if (token0 == router.WETH()) {
          router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(AMOUNTOUTMIN, innerPath, recipient, deadline);
        } else {
          router.swapExactTokensForTokensSupportingFeeOnTransferTokens(AMOUNTIN, AMOUNTOUTMIN, innerPath, recipient, deadline);
        }
      }
  ...
  function swapTokensForExactETH(
    ...
    address[] memory path = getPossiblePath(token, router.WETH());
  ...
  function swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    if (path.length == 2) {
      ...
      for (uint256 i = 1; i < path.length; i++) {
        ...
        if (token1 == router.WETH()) {
          router.swapExactTokensForETHSupportingFeeOnTransferTokens(AMOUNTIN, AMOUNTOUT, innerPath, recipient, deadline);
        } else {
          router.swapExactTokensForTokensSupportingFeeOnTransferTokens(AMOUNTIN, AMOUNTOUT, innerPath, recipient, deadline);
        }
      }
    }
  }
  ...
  function swapETHForExactTokens(
    ...
    if (path.length == 2) {
      router.swapETHForExactTokens{value: msg.value}(amountOut, path, to, deadline);
    } else {
      for (uint256 i = 1; i < path.length; i++) {
        address token0 = path[i - 1];
        address token1 = path[i];
        ...
        uint256 AMOUNTIN = token0 == router.WETH() ? msg.value : IERC20(token0).balanceOf(address(this));
        uint256[] memory amounts = router.getAmountsOut(AMOUNTIN, innerPath);
        uint256 AMOUNTOUT = token1 == token ? amountOut : amounts[amounts.length - 1];
        address recipient = i == path.length - 1 ? to : address(this);

        if (token0 == router.WETH()) {
          router.swapETHForExactTokens{value: msg.value}(AMOUNTOUT, innerPath, recipient, deadline);
        } else {
          router.swapTokensForExactTokens(AMOUNTIN, AMOUNTOUT, innerPath, recipient, deadline);
        }
      }
    }
  }
