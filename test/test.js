...
describe("Tests", () => {
  ...
  describe("Staking Pool", () => {
    ...
    before(async () => {
      ...
      const ERC20Factory = await ethers.getContractFactory("TestERC20");
      erc201 = await ERC20Factory.deploy(ethers.utils.parseEther("180000"));
      erc201 = await erc201.deployed();

      erc202 = await ERC20Factory.deploy(ethers.utils.parseEther("180000"));
      erc202 = await erc201.deployed();

      const StakingPoolFactory = await ethers.getContractFactory("StakingPool");
      stakingPool = await StakingPoolFactory.deploy(signer1.address, erc201.address, erc202.address, 5, 1800, 8, 60 * 60 * 24 * 5);
      stakingPool = await stakingPool.deployed();

      await erc201.transfer(stakingPool.address, ethers.utils.parseEther("10400"));
      await erc202.transfer(stakingPool.address, ethers.utils.parseEther("10400"));
    });

    it("should allow to stake", async () => {
      await erc201.approve(stakingPool.address, ethers.utils.parseEther("2000"));
      await expect(stakingPool.stakeAsset(erc201.address, ethers.utils.parseEther("2000"))).to.emit(stakingPool, "Staked");
    });
    ...
    it("should allow withdrawals", async () => {
      ...
      await expect(() => stakingPool.withdrawRewards(stakeId)).to.changeTokenBalance(erc202, signer1, ethers.utils.parseUnits("193.2", 18));
    });
  });
});
